...
def _slide_dashboard_title_cover(prs):
    ...
    # Main title
    _add_text_box(slide, "Assignments Progress Report",
                  Inches(2), Inches(3.70), Inches(16.0), Inches(1.20),
                  font_size=44, bold=True, color=WHITE, align=PP_ALIGN.CENTER)

    # Subtitle
    _add_text_box(slide, "Minister Outreach",
...
def generate_pptx_all_companies_dashboard(dfs: dict, lang: str = "en", ministry_note: str = "") -> bytes:
    ...
    kpi_data = [
        ...
        ("Actions",             str(total_acts), MISA_GOLD,  "✓"),
        ("Completed",           str(n_done_a),   MISA_GREEN, "★"),
        ("In Progress",         str(n_prog_a),   MISA_GOLD,  "↺"),
        ("No of Opportunities", str(n_opps),      MISA_GREEN, "◎"),
    ]
    kcard_w = Inches(3.76)
    kcard_h = Inches(1.00)
    _dsq    = Inches(0.46)
    for i, (lbl, val, col, icon) in enumerate(kpi_data):
        ...
        _txt_kx = kx + _dsq + Inches(0.14)
        _txt_kw = kcard_w - _dsq - Inches(0.18)
        _add_text_box(slide, val, _txt_kx, Inches(1.00), _txt_kw, Inches(0.50),
                      font_size=22, bold=True, color=WHITE, align=PP_ALIGN.CENTER)
        _add_text_box(slide, lbl, _txt_kx, Inches(1.46), _txt_kw, Inches(0.38),
    ...
    _add_text_box(slide, "Actions Progress by Company",
                  _BARS_X, _CONTENT_Y, _DONUT_X - _BARS_X, Inches(0.26),
                  font_size=11, bold=True, color=DARK)

    # ── Two-column bars (left 8 / right 8 companies) ──────────────────────────
    if not actions.empty and "Company Name" in actions.columns and "Status" in actions.columns:
        ...
        _sorted_cos_list = list(_sorted_cos)

        _bar_h    = Inches(0.44)
        _bar_gap  = Inches(0.05)
        _col_gap  = Inches(0.22)
        _half_w   = (_DONUT_X - _BARS_X - _col_gap) / 2    # ~7.29"
        _col2_x   = _BARS_X + _half_w + _col_gap            # right column start
        _nm_w2    = Inches(1.70)
        _end_w2   = Inches(1.10)
        _trk_w2   = _half_w - _nm_w2 - _end_w2 - Inches(0.20)  # ~4.09"
        _max_by   = _CONTENT_BOT - _bar_h

        # Thin column separator
        _sep_x = _BARS_X + _half_w + Inches(0.06)
        _add_rect(slide, _sep_x, _CONTENT_Y + Inches(0.30),
                  Inches(0.01), _max_by - (_CONTENT_Y + Inches(0.30)),
        ...
        for _col_x, _col_cos in [
            ...
            for _co_nb, (_dn, _pr, _ns, _tot) in _col_cos:
                ...
                # Company name
                _add_text_box(slide, str(_co_nb)[:20], _col_x, _by,
                              _nm_w2, _bar_h,
                              font_size=11, bold=True, color=DARK)

                # 3-segment bar
                _bx  = _col_x + _nm_w2 + Inches(0.08)
                _by2 = _by + Inches(0.08)
                _bsh = _bar_h - Inches(0.16)

                _add_rect(slide, _bx, _by2, _trk_w2, _bsh,
                          fill_color=_rgb("#D0D0D0"), line_color=_rgb("#D0D0D0"))
                if _dn > 0 and _tot > 0:
                    _dw = _trk_w2 * _dn / _tot
                ...
                if _pr > 0 and _tot > 0:
                    _px = _bx + _trk_w2 * _dn / _tot
                    _pw = _trk_w2 * _pr / _tot
                    ...
                    if _pw > Inches(0.38):
                        _add_text_box(slide, f"{round(_pr/_tot*100)}%",
                                      ...
                                      font_size=8, bold=True, color=WHITE, align=PP_ALIGN.CENTER)
                if _ns > 0 and _tot > 0:
                    _nw = _trk_w2 * _ns / _tot
                    _nx = _bx + _trk_w2 * (_dn + _pr) / _tot
                    if _nw > Inches(0.38):
                        _add_text_box(slide, f"{round(_ns/_tot*100)}%",
                                      _nx, _by2, _nw, _bsh,
                                      font_size=8, bold=True, color=_rgb("#555555"), align=PP_ALIGN.CENTER)

                # Date range (compact, below company name)
                ...
                if _dates_txt:
                    _add_text_box(slide, _dates_txt,
                                  _col_x + Inches(0.02), _by + Inches(0.26),
                                  _nm_w2 - Inches(0.04), Inches(0.16),
                                  font_size=6.5, color=_rgb("#888888"))

                # Bar-end: Sector | Country / N opps
                _ex = _bx + _trk_w2 + Inches(0.06)
                ...
                _be_half = _bar_h // 2
                _add_text_box(slide, _sec_cty, _ex, _by, _end_w2, _be_half,
                              font_size=9, color=_rgb("#888888"))
                _n_opps_co = len(opportunities[opportunities["Company Name"] == _co_nb]) if not opportunities.empty and "Company Name" in opportunities.columns else 0
                _add_text_box(slide, f"{_n_opps_co} opp{'s' if _n_opps_co != 1 else ''}",
                              _ex, _by + _be_half, _end_w2, _be_half,
    ...
    _add_text_box(slide, "Companies by Sector",
    ...
    _add_text_box(slide, "Companies by Country",
    ...
    # ── Bottom strip: LEFT = Opportunities by Sector, RIGHT = IMPORTANT ACTIVATES
    _IA_Y     = _CONTENT_BOT
    _IA_H     = _IA_STRIP_H
    _IA_TTL_H = Inches(0.28)
    _HALF_X   = SLIDE_W / 2

    # ── LEFT: Opportunities by Sector ────────────────────────────────────────
    _OPP_X = Inches(0)
    _OPP_W = _HALF_X
    _add_rect(slide, _OPP_X, _IA_Y, _OPP_W, _IA_TTL_H,
              fill_color=_rgb("#1B5C3F"), line_color=_rgb("#1B5C3F"))
    _add_rect(slide, _OPP_X, _IA_Y, Inches(0.05), _IA_TTL_H,
              fill_color=_rgb("#C89B3C"), line_color=_rgb("#C89B3C"))
    _add_text_box(slide, "OPPORTUNITIES BY SECTOR",
                  _OPP_X + Inches(0.12), _IA_Y + Inches(0.04),
                  _OPP_W - Inches(0.15), _IA_TTL_H - Inches(0.06),
                  font_size=9, bold=True, color=_rgb("#C89B3C"))
    _add_rect(slide, _OPP_X, _IA_Y + _IA_TTL_H, _OPP_W, _IA_H - _IA_TTL_H,
              fill_color=_rgb("#FAF7EE"), line_color=_rgb("#D8D2C0"))

    _opp_sec: dict = {}
    if not opportunities.empty and "Company Name" in opportunities.columns:
        for _, _opr in opportunities.iterrows():
            _co_n3 = str(_opr.get("Company Name", "") or "").strip()
            _inv_m = investors[investors["Company Name"] == _co_n3] if not investors.empty and "Company Name" in investors.columns else pd.DataFrame()
            if not _inv_m.empty:
                _sec3 = str(_inv_m.iloc[0].get("Sector", "") or "").strip()
                if _sec3 and _sec3.lower() not in ("nan", "none", "-", "n/a"):
                    _opp_sec[_sec3] = _opp_sec.get(_sec3, 0) + 1
    _opp_sec_items = sorted(_opp_sec.items(), key=lambda x: x[1], reverse=True)[:6]
    if _opp_sec_items:
        _max_opp = max(c for _, c in _opp_sec_items)
        _bz_x  = _OPP_X + Inches(0.12)
        _bz_w  = _OPP_W - Inches(0.24)
        _lbl_w3 = Inches(1.70)
        _cnt_w3 = Inches(0.38)
        _trk_w3 = _bz_w - _lbl_w3 - _cnt_w3 - Inches(0.08)
        _bh3    = Inches(0.09)
        _n_sec3 = len(_opp_sec_items)
        _row_h3 = (_IA_H - _IA_TTL_H - Inches(0.06)) / max(_n_sec3, 1)
        _pal3   = ["#1B5C3F", "#2D7A54", "#3D9068", "#C9974A", "#E0B06A", "#888888"]
        for _si3, (_sname3, _scnt3) in enumerate(_opp_sec_items):
            _ry3 = _IA_Y + _IA_TTL_H + Inches(0.03) + _si3 * _row_h3 + (_row_h3 - _bh3) / 2
            _add_text_box(slide, str(_sname3)[:18],
                          _bz_x, _ry3 - Inches(0.03),
                          _lbl_w3, _bh3 + Inches(0.06),
                          font_size=7.5, color=_rgb("#2B2B2B"))
            _add_rect(slide, _bz_x + _lbl_w3, _ry3, _trk_w3, _bh3,
                      fill_color=_rgb("#E8E5DC"), line_color=_rgb("#E8E5DC"))
            _fw3 = _trk_w3 * _scnt3 / max(_max_opp, 1)
            _add_rect(slide, _bz_x + _lbl_w3, _ry3, _fw3, _bh3,
                      fill_color=_rgb(_pal3[_si3 % len(_pal3)]), line_color=_rgb(_pal3[_si3 % len(_pal3)]))
            _add_text_box(slide, str(_scnt3),
                          _bz_x + _lbl_w3 + _trk_w3 + Inches(0.04), _ry3 - Inches(0.03),
                          _cnt_w3, _bh3 + Inches(0.06),
                          font_size=7.5, bold=True, color=_rgb("#2B2B2B"))
    else:
        _add_text_box(slide, "No opportunity data available.",
                      _OPP_X + Inches(0.15), _IA_Y + _IA_TTL_H + Inches(0.10),
                      _OPP_W - Inches(0.20), Inches(0.22),
                      font_size=8, color=_rgb("#888888"))

    # ── RIGHT: IMPORTANT ACTIVATES ────────────────────────────────────────────
    _IA_X = _HALF_X
    _IA_W = SLIDE_W - _HALF_X
    ...
    # Collect high-priority pending/in-progress actions (YES-flagged, max 4)
    _ia_items = []
    try:
    ...
    if ministry_note and ministry_note.strip():
        ...
        _add_text_box(slide, f"► {ministry_note.strip()[:120]}",
                      _IA_X + Inches(0.18), _ia_content_y + Inches(0.04),
                      _IA_W - Inches(0.36), _note_h - Inches(0.08),
                      font_size=10, bold=True, color=_rgb("#0B4A2F"))
        _ia_content_y += _note_h + Inches(0.04)

    _ia_item_y = _ia_content_y
    _ia_col_w  = (_IA_W - Inches(0.20)) / max(min(len(_ia_items), 2), 1)
    _ia_rows   = [_ia_items[i:i+2] for i in range(0, len(_ia_items), 2)]
    ...
    for _ir_idx, _ia_row in enumerate(_ia_rows[:2]):
        ...
        for _ic_idx, (_co_n, _ac_n, _pr_n) in enumerate(_ia_row):
            ...
            _add_text_box(slide, f"{_co_n}  ·  {_ac_n}",
                          ...
                          font_size=10, color=_rgb("#1A1A1A"))
            _ia_col_x += _ia_col_w
    ...
    _pg_w = Inches(0.56)
    _pg_h = Inches(0.26)
    for _pg_i, _pg_sl in enumerate(prs.slides, 1):
        _tb = _pg_sl.shapes.add_textbox(
            SLIDE_W - _pg_w - Inches(0.14), SLIDE_H - _pg_h - Inches(0.08),
            _pg_w, _pg_h)
        _tf = _tb.text_frame
        _tf.paragraphs[0].alignment = PP_ALIGN.RIGHT
        _rn = _tf.paragraphs[0].add_run()
        _rn.text = str(_pg_i)
        _rn.font.size = Pt(9)
        _rn.font.color.rgb = _rgb("#AAAAAA")
...
def _co_slide_cover_profile(prs, company, inv_row, opps, acts, meetings, deals, lang, _skip_logos=False, _skip_charts=False):
    ...
    # Breadcrumb
    _add_text_box(slide,
                  "Ministry of Investment  ·  Minister Office  ·  Minister Outreach  ·  Assignments Progress Report",
    ...
    # ── Journey Timeline: y=4.145", w=8.854" — action circles on date axis ─────
    TL_X = Inches(0.417)
    TL_Y = Inches(4.145)
    TL_W = Inches(8.854)
    TL_H = Inches(1.771)
    ...
    _TL_PAD_X = Inches(0.45)
    _TL_AX_X  = TL_X + _TL_PAD_X
    _TL_AX_W  = TL_W - _TL_PAD_X * 2
    AX_Y = TL_Y + Inches(1.05)

    # Horizontal axis line
    _add_rect(slide, _TL_AX_X, AX_Y, _TL_AX_W, Inches(0.012),
              fill_color=_rgb("#CCCCCC"), line_color=_rgb("#CCCCCC"))

    try:
        _today = date.today()

        # Build action list with dates + progress
        _tl_acts = []
        if not acts.empty:
            _ac = acts.copy()
            _ac["_start"] = pd.to_datetime(_ac.get("Start Date", pd.Series(dtype=str)), errors="coerce")
            _ac["_due"]   = pd.to_datetime(_ac.get("Due Date",   pd.Series(dtype=str)), errors="coerce")
            for _i, (_idx, _ar) in enumerate(_ac.iterrows()):
                _d = _ar["_due"].date() if pd.notna(_ar["_due"]) else None
                if _d is None and pd.notna(_ar["_start"]):
                    _d = _ar["_start"].date()
                if _d is None:
                    continue
                _raw_p = _ar.get("Progress", 0)
                try:
                    _pv = float(str(_raw_p).replace("%", "")) if _raw_p not in (None, "", "nan") else 0.0
                    if _pv > 1.0: _pv /= 100.0
                except Exception:
                    _pv = 0.0
                if str(_ar.get("Status", "")).lower() in ("completed", "cancelled"):
                    _pv = 1.0
                _tl_acts.append({"num": _i + 1, "due": _d, "progress": _pv})

        if _tl_acts:
            _all_d = [a["due"] for a in _tl_acts]
            _vt_min = min(_all_d).replace(day=1)
            _vt_max_raw = max(_all_d)
            _ld = _calendar.monthrange(_vt_max_raw.year, _vt_max_raw.month)[1]
            _vt_max = _vt_max_raw.replace(day=_ld)
            if _today < _vt_min:
                _vt_min = _today.replace(day=1)
            if _today > _vt_max:
            ...
            def _tlx(d):
                return _TL_AX_X + max(0.0, min(1.0, (d - _vt_min).days / _span)) * _TL_AX_W

            # Month tick marks + labels
            _n_mo = (_vt_max.year - _vt_min.year) * 12 + (_vt_max.month - _vt_min.month)
            _show_every = 1 if _n_mo <= 8 else (2 if _n_mo <= 16 else 3)
            _mo = _vt_min.replace(day=1)
            _mo_count = 0
            while _mo <= _vt_max and _mo_count < 40:
                _mo_count += 1
                _mx = _tlx(_mo)
                _add_rect(slide, _mx, AX_Y, Inches(0.010), Inches(0.09),
                          fill_color=_rgb("#AAAAAA"), line_color=_rgb("#AAAAAA"))
                if _mo_count % _show_every == 0:
                    _mlbl = _mo.strftime("%b '%y") if _n_mo > 12 else _mo.strftime("%b")
                    _add_text_box(slide, _mlbl,
                                  _mx - Inches(0.30), AX_Y + Inches(0.10),
                                  Inches(0.60), Inches(0.24),
                                  font_size=7.5, color=_rgb("#666666"), align=PP_ALIGN.CENTER)
                _mo = (_mo.replace(year=_mo.year + 1, month=1)
                       if _mo.month == 12 else _mo.replace(month=_mo.month + 1))

            # TODAY marker — small dark circle on line
            _nx = _tlx(_today)
            _now_r = Inches(0.09)
            _now_sh = slide.shapes.add_shape(9,
                int(_nx - _now_r), int(AX_Y - _now_r), int(_now_r * 2), int(_now_r * 2))
            _now_sh.fill.solid(); _now_sh.fill.fore_color.rgb = DARK
            _now_sh.line.fill.background()
            # Tick + label above
            _add_rect(slide, _nx - Inches(0.005), AX_Y - Inches(0.36),
                      Inches(0.010), Inches(0.36),
                      fill_color=DARK, line_color=DARK)
            _add_text_box(slide, "TODAY",
                          _nx - Inches(0.26), AX_Y - Inches(0.52),
                          Inches(0.52), Inches(0.17),
                          font_size=6.5, bold=True, color=DARK, align=PP_ALIGN.CENTER)

            # Action circles — small, side-by-side, no text labels
            _circ_r  = Inches(0.17)
            _circ_y  = AX_Y - Inches(0.52)
            _min_gap = int(_circ_r * 2 + Inches(0.03))
            _sorted_acts = sorted(_tl_acts, key=lambda a: a["due"])
            _positions = []
            for _a in _sorted_acts:
                _nat = int(_tlx(_a["due"]))
                _placed = False
                for _cand in ([_nat] +
                              [_nat + _d for _d in range(_min_gap, _min_gap * 10, _min_gap)] +
                              [_nat - _d for _d in range(_min_gap, _min_gap * 10, _min_gap)]):
                    if all(abs(_cand - _px) >= _min_gap for _px, _ in _positions):
                        _positions.append((_cand, _a)); _placed = True; break
                if not _placed:
                    _positions.append((_nat + len(_positions) * _min_gap, _a))

            for _cx, _a in _positions:
                if _a["progress"] >= 1.0:
                    _cfill = _rgb("#1B5C3F")
                elif _a["progress"] > 0:
                    _cfill = _rgb(MISA_GOLD)
                else:
                    _cfill = _rgb("#BBBBBB")
                # Connector to axis
                _conn_h = int(AX_Y - Inches(0.01)) - int(_circ_y + _circ_r)
                if _conn_h > 0:
                    _cn = slide.shapes.add_shape(1,
                        int(_cx - Inches(0.005)), int(_circ_y + _circ_r),
                        int(Inches(0.01)), _conn_h)
                    _cn.fill.solid(); _cn.fill.fore_color.rgb = _rgb("#CCCCCC")
                    _cn.line.fill.background()
                # Circle
                _circ = slide.shapes.add_shape(9,
                    int(_cx - _circ_r), int(_circ_y - _circ_r),
                    int(_circ_r * 2), int(_circ_r * 2))
                _circ.fill.solid(); _circ.fill.fore_color.rgb = _cfill
                _circ.line.fill.background()
                # Number
                _add_text_box(slide, str(_a["num"]),
                              int(_cx - _circ_r), int(_circ_y - _circ_r),
                              int(_circ_r * 2), int(_circ_r * 2),
                              font_size=7, bold=True, color=WHITE, align=PP_ALIGN.CENTER)

            # Legend
            _leg_y = TL_Y + TL_H - Inches(0.22)
            for _li, (_llbl, _lcol) in enumerate([
                ("Complete",   "#1B5C3F"),
                ("In Progress", MISA_GOLD),
                ("Not Started", "#BBBBBB"),
            ]):
                _lx = TL_X + Inches(0.20) + _li * Inches(2.70)
                _ldot = slide.shapes.add_shape(9,
                    int(_lx), int(_leg_y), int(Inches(0.13)), int(Inches(0.13)))
                _ldot.fill.solid(); _ldot.fill.fore_color.rgb = _rgb(_lcol)
                _ldot.line.fill.background()
                _add_text_box(slide, _llbl,
                              int(_lx + Inches(0.18)), int(_leg_y - Inches(0.01)),
                              Inches(2.40), Inches(0.18),
                              font_size=7, color=_rgb("#666666"))
    except Exception:
        pass
    ...
    for _, _rrow in _acts_ordered.iterrows():
        ...
        _add_text_box(slide, _desc2, _cx_cur + Inches(0.05), _row_y - Inches(0.01),
        ...
        _add_text_box(slide, _edate, _cx_cur + Inches(0.04), _row_y + Inches(0.07),
                      ...
                      font_size=13, color=_rgb("#2B2B2B"), align=PP_ALIGN.CENTER)
        _cx_cur += CW_TBL[4]
        # Col 5: Update — verbatim from RM Input column
        _add_rect(slide, _cx_cur, _row_y, CW_TBL[5], ROW_H2, fill_color=_alt, line_color=_rgb("#E0E0DC"))
        _rm_raw = str(_rrow.get("RM Input", "") or "").strip()
        _upd2 = _rm_raw if _rm_raw and _rm_raw.lower() not in ("nan", "none", "-", "n/a") else "—"
